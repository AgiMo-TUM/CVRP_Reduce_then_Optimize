...
def _generate_runtime_missing_arc_plot_impl(benchmarking_path, with_bars=False):
    ...
    for folder_name in os.listdir(benchmarking_path):
        ...
        for file_name in os.listdir(folder_path):
            ...
            middle = parts[-2] + "_" + parts[-1].split(".")[0]

            # Populate per-instance baseline cost from the result dict itself.
            if middle not in cost_dict and "exact_objective_value" in result_dict:
                # print("problem")
    ...
    for key in performance_dict.keys():
        ...
        parts = key.split("_")
        middle = parts[-2] + "_" + parts[-1].split(".")[0]
        optimal_value = cost_dict.get(middle)
        ...
        for threshold in performance_dict[key]:
            ...
            # new_performance_dict[key].append((threshold[0], ((threshold[1][0]- optimal_value)*100/optimal_value,
            #                                         threshold[1][1] , threshold[1][2]/5050, threshold[1][4])))
        if not found_threshold :
            for threshold in performance_dict[key]:
                if threshold[0] != 1 :
                    renew_performance_dict[key].append((threshold[0], ((threshold[1][0]-optimal_value)*100/optimal_value,
    ...
    filtered_param_values = [p for p in param_values]
    ...
    for p in param_values:
        ...
        # # Compute mean and std on trimmed data
        avg_runtimes.append(np.mean(arr))
        std_runtimes.append(np.std(arr))
    ...
    ax1.set_title("Average complete relative Gap and mean relative Runtime per Threshold for unsolved CVRP")
...
def _generate_runtime_missing_arc_plot_impl_knn_compare(benchmarking_path, benchmarking_knn_path):
    """Aggregates benchmarking results into per-threshold mean/median value-gap and runtime curves with shaded std bands, then draws histograms of gap and runtime per threshold."""
    performance_dict = {}
    better_runtime_dict = {}
    out_runtime_limit_dict = {}
    ...
    for folder_name in os.listdir(benchmarking_path):
        ...
        for file_name in os.listdir(folder_path):
            ...
            if middle not in cost_dict and "exact_objective_value" in result_dict:
                # print("problem")
                cost_dict[middle] = result_dict["exact_objective_value"]

            if result_dict["solver_value"] > 0 :
                performance_dict[file_name].append(
                    (float(folder_name)+1,(result_dict["solver_value"],
                    result_dict["runtime"],
                    result_dict["num_arcs_pred"],
                    result_dict["exact_objective_value"],
                    result_dict["num_arcs_enriched"]-result_dict["num_arcs_pred"]))
    ...
    # print(cost_dict)
    for folder_name in os.listdir(benchmarking_knn_path):
        folder_path = os.path.join(benchmarking_knn_path, folder_name)
        if not os.path.isdir(folder_path):
            continue
    ...
    param_values = sorted({entry[0] for values in new_performance_dict.values() for entry in values})
    param_values = [p for p in param_values if p != 1.0]
    ...
    # small_vals = [ 0.05, 0.1, 0.2, 0.266667, 0.33333]
    small_vals = [ 3, 5, 9, 17, 21, 31]
    large_vals = [ 2.0, 4.0, 8.0, 16.0, 20.0, 30.0]

    # Create pairs for x-axis labels
    pairs = list(zip(small_vals, large_vals))
    x_labels = [f"({s},{l})" for s, l in pairs]

    # Compute metrics for each group
    avg_values_small   = np.array([np.mean(value_acc[p]) for p in small_vals])
    avg_values_large   = np.array([np.mean(value_acc[p]) for p in large_vals])

    median_values_small = np.array([np.median(value_acc[p]) for p in small_vals])
    median_values_large = np.array([np.median(value_acc[p]) for p in large_vals])

    std_values_small = np.array([np.std(value_acc[p]) for p in small_vals])
    std_values_large = np.array([np.std(value_acc[p]) for p in large_vals])

    std_runtimes_small = np.array([
        np.std([runtime_acc[p]])
        for p in small_vals
    ])

    std_runtimes_large = np.array([
        np.std([runtime_acc[p]])
        for p in large_vals
    ])
    ...
    avg_runtimes_small = []
    avg_runtimes_large = []
    median_runtimes_small = []
    median_runtimes_large = []
    ...
    for p in small_vals:
        ...
        cutoff = np.percentile(arr, 99)
        trimmed = arr[arr <= cutoff]
        avg_runtimes_small.append(np.mean(trimmed))
        median_runtimes_small.append(np.median(arr))
    ...
    for p in large_vals:
        ...
        cutoff = np.percentile(arr, 99)
        trimmed = arr[arr <= cutoff]
        avg_runtimes_large.append(np.mean(trimmed))
        median_runtimes_large.append(np.median(arr))

    avg_runtimes_small = np.array(avg_runtimes_small)
    avg_runtimes_large = np.array(avg_runtimes_large)
    median_runtimes_small = np.array(median_runtimes_small)
    median_runtimes_large = np.array(median_runtimes_large)
    ...
    # --- RVG curves (left axis) ---
    ax1.plot(x_positions, avg_values_small, marker='o', color='tab:blue', label="Avg RVG (GNN)")
    ax1.plot(x_positions, avg_values_large, marker='o', color='tab:cyan', label="Avg RVG (knn)")

    ax1.plot(x_positions, median_values_small, linestyle='--', color='navy', label="Median RVG (GNN)")
    ax1.plot(x_positions, median_values_large, linestyle='--', color='teal', label="Median RVG (knn)")

    # --- RRG curves (right axis) ---
    ax2.plot(x_positions, avg_runtimes_small, marker='x', linestyle='--', color='tab:red', label="Avg RRG (GNN)")
    ax2.plot(x_positions, avg_runtimes_large, marker='x', linestyle='--', color='tab:orange', label="Avg RRG (knn)")

    ax2.plot(x_positions, median_runtimes_small, linestyle=':', color='darkred', label="Median RRG (GNN)")
    ax2.plot(x_positions, median_runtimes_large, linestyle=':', color='brown', label="Median RRG (knn)")

    ax1.set_xlabel("Threshold pairs (GNN, knn)")
    ax1.set_ylabel("RVG gap %", color='tab:blue')
    ax2.set_ylabel("RRG gap %", color='tab:red')

    ax1.set_title("RVG and RRG gaps for paired thresholds")

    # Combined legend (split into two boxes)
    lines1, labels1 = ax1.get_legend_handles_labels()
    lines2, labels2 = ax2.get_legend_handles_labels()
    ...
    legend_right = ax1.legend(
        all_lines[mid:],
        all_labels[mid:],
        loc="upper left",
        bbox_to_anchor=(0.42, 1.0)   # adjust spacing horizontally
    )

    ax1.add_artist(legend_left)
    ax1.add_artist(legend_right)

    ax1.fill_between(
        x_positions,
        avg_values_small - std_values_small,
        avg_values_small + std_values_small,
        color='tab:blue', alpha=0.2
    )
    ...
    ax1.fill_between(
        x_positions,
        avg_values_large - std_values_large,
        avg_values_large + std_values_large,
    ...
    ax2.fill_between(
        x_positions,
        avg_runtimes_small - std_runtimes_small,
        avg_runtimes_small + std_runtimes_small,
        color='tab:red', alpha=0.2
    )
    ...
    ax2.fill_between(
        x_positions,
        avg_runtimes_large - std_runtimes_large,
        avg_runtimes_large + std_runtimes_large,
...
def _generate_runtime_missing_arc_plot_impl_TW(benchmarking_path, with_bars=False):
    """Aggregates benchmarking results into per-threshold mean/median value-gap and runtime curves with shaded std bands, then draws histograms of gap and runtime per threshold."""
    performance_dict = {}
    better_runtime_dict = {}
    out_runtime_limit_dict = {}
    ...
    for key in performance_dict.keys():
        ...
        for threshold in performance_dict[key]:
            if threshold[0] == 1.0 :
                found_threshold  = True
                optimal_runtime = threshold[1][1]
                if (optimal_runtime < 5000):
                    optimal_value = threshold[1][0]
                break
        for threshold in performance_dict[key]:
            new_performance_dict[key].append((threshold[0], ((threshold[1][0]- optimal_value)*100/optimal_value,
                                                    (threshold[1][1]-optimal_runtime)*100/optimal_runtime , threshold[1][2]/5050, threshold[1][4])))
...
def generate_runtime_missing_arc_plot_with_size_bars(benchmarking_path):
    """Aggregates benchmarking results into per-threshold mean/median value-gap and runtime curves with shaded std bands, then draws histograms of gap and runtime per threshold."""
    performance_dict = {}
    ...
    filtered_param_values = [p for p in param_values if p !=1]
    ...
    # ============================================================
    #  THIRD AXIS FOR GREEN BARS (does NOT affect y-axis scaling)
    # ============================================================
    ax3 = ax1.twinx()
    ax3.set_ylim(0, 1)                     # dummy range so bars fit visually
    ax3.get_yaxis().set_visible(False)     # hide the y-axis completely

    # --- Bars for predicted arcs ---
    bar_width = 0.35
    bars = ax3.bar(
    ...
    for i, val in enumerate(avg_pred_arcs):
        ax3.text(
    ...
    # ============================================================
    #  BLUE CURVE (value gap) — LEFT AXIS
    # ============================================================
    ax1.plot(
    ...
    # ============================================================
    #  RED CURVE (runtime gap) — RIGHT AXIS
    # ============================================================
    ax2.plot(
    ...
    ax1.set_title("Average RVG, RRG, and Predicted Arcs per Threshold")
...
def generate_new_HGS_77_reduction_plot_2(benchmarking_path_list,  save_path):


    HGS_time_limit_dict_sol = {}
    # inf_r_dict = {}
    # for folder_name in os.listdir(benchmarking_path_inf_r):
    #     folder_path = os.path.join(benchmarking_path_inf_r, folder_name)
    #     if not os.path.isdir(folder_path):
    #         continue

    #     nums = re.findall(r"\d+", folder_name)
    #     nums_name_list = list(map(int, nums[-3:])) 

    #     # Loop over instance files
    #     i = 0
    #     for file_name in os.listdir(folder_path + r"\0.99"):
    #         if i > 2:
    #             break
    #         i += 1
    #         file_path = os.path.join(folder_path + r"\0.99", file_name)

    #         if file_name not in inf_r_dict.keys():
    #             inf_r_dict[file_name] = {}


    #         with gzip.open(file_path, "rb") as f:
    #             result_dict = pkl.load(f)
    #             #result_dict = load_with_numpy_fix(f)

    #         print(file_name, ", ",result_dict["completion_runtime"], ", ", result_dict["lower_bound"], ", ", (nums_name_list[1], nums_name_list[2]) )

    #         inf_r_dict[file_name]= [result_dict["completion_runtime"], result_dict["lower_bound"]]
    #     print("done")
    ...
    for benchmarking_path in benchmarking_path_list:
        for folder_name in os.listdir(benchmarking_path):
            ...
            # print(folder_path)
            # print(nums)
            # print(nums_name_list)
            numbers = re.findall(r'\d+', folder_path)
            last_two = list(map(int, numbers[-2:]))
            print(last_two)
            # Loop over instance files
            i=0
            for file_name in os.listdir(folder_path):
                ...
                if file_name not in HGS_time_limit_dict_sol.keys():
                    HGS_time_limit_dict_sol[file_name] = {}
                ...
                print(file_name, ", ",result_dict["runtime"], ", ", (last_two[0], last_two[1]), result_dict["inference_runtime"],
                    result_dict["build_solver_runtime"])

                if True:
                    HGS_time_limit_dict_sol[file_name][(last_two[0], last_two[1])] = (
                        result_dict["num_arcs_pred"], result_dict["instance_log_HGS_dict"][last_two[1]],
                        result_dict["runtime"], result_dict["inference_runtime"], result_dict["build_solver_runtime"]
                    )
                else :
                    break
            print("done")
    ...
    print("Saving aggregated results to:", save_path)
    with open(save_path, "wb") as f:
        pkl.dump(HGS_time_limit_dict_sol, f)
    return HGS_time_limit_dict_sol
...
def plt_new_HGS_77_average_runtime_2(dict_path):
    ...
    for key in HGS_time_limit_dict_sol.keys():
        ...
        for key_2 in HGS_time_limit_dict_sol[key]:
            ...
            last_value=filtered_list[0][2]
            new_average_list= []
            time_list = [i+1 for i in range(2000)]
            print("build_time = ",HGS_time_limit_dict_sol[key][key_2][4] )
            print("inference_time = ",HGS_time_limit_dict_sol[key][key_2][3] )
            initial_time = HGS_time_limit_dict_sol[key][key_2][4] + HGS_time_limit_dict_sol[key][key_2][3] -2000
            print("initial_time : ", initial_time)
            # print("first_last_time : ", last_time)
            for t in range(len(filtered_list)):
                # compte_time_step = (int(filtered_list[t][1].rstrip("s"))-last_time)//5
                # for i in range(compte_time_step):
                # print("last_value : ", last_value)
                # print("longest_value : ", longest_value)
                new_relative_value = (last_value-longest_value)*100/longest_value
                new_average_list.append(new_relative_value)
                last_value=filtered_list[t][2]
                new_last_time=int(filtered_list[t][1].rstrip("s"))
                # print("time_space : ", new_last_time - last_time)
                for j in range(new_last_time - last_time-1):
                    new_average_list.append(new_relative_value)
                last_time=new_last_time
            print("last last_time : ", last_time)
            while len(new_average_list)<2000:
                new_average_list.append(new_average_list[-1])

            new_relative_HGS_dict[key][1].append({key_2 : [new_average_list, initial_time]})
        # print(new_relative_HGS_dict)
    ...
    for subkey in sorted(avg_curves.keys()):
        if subkey not in selected and subkey != (100, 1):
            continue
...
def get_performances_selection_method(model_path, instance_dir, is_time_windows,
                                      start_index, end_index, threshold_type, thresholds_list = None):
    ...
    for counter, instance_path in enumerate(instance_paths):
        ...
        for threshold in thresholds_list:
            ...
            best_known_solution_array = []
            for (u,v) in  instance_solution.keys():
                best_known_solution_array.append(instance_solution[(u,v)])

            best_known_solution_array =np.array(best_known_solution_array)
            # print(relevant_connections)
            # print(best_known_solution_array)
    ...
    for key, tuple_list in performance_dict.items():
        arr = np.array(tuple_list, dtype=float)   # shape (N, 4)
        avg_tuple = tuple(arr.mean(axis=0))       # average each column
        avg_dict[key] = avg_tuple
    # print(avg_dict)
    clean_dict = {
    key: tuple(round(float(x), 3) for x in vals)
        for key, vals in avg_dict.items()
    }
