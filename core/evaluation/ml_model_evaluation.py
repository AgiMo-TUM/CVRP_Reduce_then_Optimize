...
def evaluate_prediction_model(
    ...
    # Features
    if features == "graph_raw":
        x_nodes, x_connections = get_graph_raw_features(
            sample_paths,
        )
        x = (x_nodes, x_connections)
    # elif features in [
    #     "combined_raw_arc_features",
    #     "advanced_arc_features",
    #     "advanced_arc_features_plus_stat_features",
    # ]:
    #     x_connections = get_arc_features(sample_paths, features=features)
    #     x = (x_connections,)
    else:
        raise ValueError
...
def generalization_performance_comparison_across_datasets(
    ...
    for config, (chkpnt_path, transfer_data) in transfer_combinations.items():
        try:
            if not cross_val:
                model = load_arc_predictor_model(
                    chkpnt_path, get_feature_fun=False, get_label_fun=False
                )
                model_config = torch.load(chkpnt_path, map_location="cpu")[
                    "model_config"
