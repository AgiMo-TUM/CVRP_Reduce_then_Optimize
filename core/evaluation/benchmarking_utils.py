    ...
    for root, _, files in os.walk(solution_dir):
        for solution_file in files:
            ...
            with gzip.open(os.path.join(root, solution_file), "rb") as f:
                result_dict = pkl.load(f)
            # predicted
            if "num_arcs_pred" in result_dict:
                num_edges_pred[method].append(result_dict["num_arcs_pred"])
            else:
                num_edges_pred[method].append(0)
            # enriched
            if "num_arcs_enriched" in result_dict:
                num_edges_enriched[method].append(result_dict["num_arcs_enriched"])
            else:
...
def get_optgaps_by_method(solution_dir, baseline="exact"):
    ...
    for root, _, files in os.walk(solution_dir):
        for solution_file in files:
            ...
            # Support both legacy ("objective_value") and current ("solver_value")
            # benchmark result schemas. Both are paired with `exact_objective_value`
            # for the gap computation downstream.
            obj_key = (
                "objective_value" if "objective_value" in result_dict
                else "solver_value" if "solver_value" in result_dict
                else None
            )
            if obj_key is not None and "exact_objective_value" in result_dict:
                obj_vals[instance_path][method] = [
                    result_dict[obj_key],
...
def get_performance_table(solution_dir, baseline="exact", epsilon=1e-4, use_gmean=True):
    ...
    # Print values
    # print("num_arcs_pred_data:", num_arcs_pred_data)
    # print("num_arcs_enriched_data:", num_arcs_enriched_data)
    # print("runtime_data:", runtime_data)
    # print("solver_runtime_data:", solver_runtime_data)
    # print("mip_gap_data:", mip_gap_data)
    # print("runtime_limit_hit_data:", runtime_limit_hit_data)
    # print("opt_gap_data:", opt_gap_data)
    # print("objval_data:", objval_data)
    ...
    num_edges_pred_dict = {
        method: np.mean(data) for method, data in num_arcs_pred_data.items()
    }
    num_edges_enriched_dict = {
        method: np.mean(data) for method, data in num_arcs_enriched_data.items()
    }
    ...
    for model_name, opt_gap_stats in opt_gap_dict.items():
        ...
        performance_dict[model_name]["#arcs predicted"] = num_edges_pred_dict[
            model_name
        ]
        performance_dict[model_name]["#arcs used"] = num_edges_enriched_dict[
            model_name
        ]
...
def get_optgap_vs_runtime_plot_with_edgeset_barplot(
    ...
    data = data.drop(baseline)
    data = data[
        ["mean", "median", "average solver runtime", "#arcs predicted"]
    ].rename(
        columns={
            "mean": "mean gap",
            "median": "median gap",
    ...
    fig.add_trace(
        go.Bar(
            x=data.index.tolist(),
            y=data["#arcs predicted"].tolist(),
            name="#arcs",
            text=data["#arcs predicted"].round().astype(int),
            # name="#edges [%]",
...
def get_num_edges_pred_vs_used_barplot(
    ...
    data = data.drop(baseline)
    data = data[["#arcs predicted", "#arcs used"]]
    ...
    fig.add_trace(
        go.Bar(
            x=data.index.tolist(),
            y=data["#arcs used"].tolist(),
            name="Actual edge set size",
            text=data["#arcs used"].round().astype(int),
    ...
    fig.update_layout(
        ...
        yaxis=dict(title="#Arcs", range=(0, data["#arcs used"].max() * 1.1)),
        barmode="overlay",
        showlegend=True,
...
def get_summary_performance_plot_across_edgesetsizes(
    ...
    benchmarking_dir: str
        Path to directory with all benchmarking results, containing a subdirectory for every method.
    experiment_dirs: dict
        Path to directory with benchmarking results for methods of interest {<method>: <dir_path>},
        containing a subdirectory for every edge set size.
    baseline: str, optional
        Method that should be used as baseline for calculating optimality gaps. Default is 'exact'.
    metric: str, optional
...
#         Indicate whether number should be absolute (True) or relative to total number of edges.
#         Default is False.
...
def get_num_arcs_by_method(solution_dir):
    """Get number of selected arcs.
    ...
    -------
    num_arcs_pred: dict
        Dictionary containing for each method a list stating the number of predicted arcs for each
        instance.
    num_arcs_enriched: dict
        Dictionary containing for each method a list stating the number of total selected arcs for
        each instance.

    """
    num_arcs_pred = defaultdict(lambda: [])
    num_arcs_enriched = defaultdict(lambda: [])
    for root, _, files in os.walk(solution_dir):
        for solution_file in files:
            method = os.path.relpath(root, solution_dir)
            with gzip.open(os.path.join(root, solution_file), "rb") as f:
            ...
            # predicted
            if "num_arcs_pred" in result_dict:
                num_arcs_pred[method].append(result_dict["num_arcs_pred"])
            else:
                num_arcs_pred[method].append(0)
            # enriched
            if "num_arcs_enriched" in result_dict:
                num_arcs_enriched[method].append(result_dict["num_arcs_enriched"])
            else:
                num_arcs_enriched[method].append(0)

    return num_arcs_pred, num_arcs_enriched


def get_num_arcs_used_by_method(solution_dir):
    """Get number of used arcs.
    ...
    """
    num_arcs_used = defaultdict(lambda: [])
    for root, _, files in os.walk(solution_dir):
        for solution_file in files:
            method = os.path.relpath(root, solution_dir)
            with gzip.open(os.path.join(root, solution_file), "rb") as f:
            ...
            if "solution" in result_dict:
                ...
                for obj in solution_dict:
                    if solution_dict[obj]==1 :
                        used_arc_count +=1
                num_arcs_used[method].append(used_arc_count)
            else:
                num_arcs_used[method].append(0)


    return num_arcs_used