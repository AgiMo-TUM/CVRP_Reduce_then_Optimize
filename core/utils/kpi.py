...
##Changes
""" Functions for evaluating CVRP instances and results. """
##End changes
...
def evaluate_solution_costs_dict(solution, arc_costs_dict):
    """Compute total costs for a CVRP solution provided as dict.

    Parameters
    ----------
    solution: dict
        Solution dictionary containing booleans for tha activation of arc (i,j).

    arc_costs_dict: dict
       Arc costs for the activation of arc from i to j with key (i,j).
    ...
    return sum([v * arc_costs_dict[(i,j)] for (i,j), v in solution.items()])


def evaluate_solution_costs_list(solution_list, arc_costs, batch=True):
    """Compute total costs for a VCRP solution provided as matrix.

    Parameters
    ----------
    solution: 1D np.array
        Solution list containing booleans for the activation of the ith arc.

    arc_costs: 1D np.array
        Arc costs for activating the ith arc.
...
def eval_arc_prediction_accuracy(pred, y):
